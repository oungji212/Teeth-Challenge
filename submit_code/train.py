...
# Model
def Train_Model(which_model='effi5', img_size=1280, weight_dir): # 모델 및 pretrained weight 불러오기
	if which_model=='effi5':
		...
		pretrained_weight = torch.load(weight_dir+'/tf_efficientdet_d5-ef44aea8.pth')
		...
		config.norm_kwargs = dict(eps=.001, momentum=.01) 
		net = EfficientDet(config, pretrained_backbone=False)
		pretrained_weight = torch.load(weight_dir+'/tf_efficientdet_d4-5b370b7a.pth')
		net.load_state_dict(pretrained_weight)
		...
		config.norm_kwargs = dict(eps=.001, momentum=.01) 
		net = EfficientDet(config, pretrained_backbone=False)
		pretrained_weight = torch.load(weight_dir+'/tf_efficientdet_d6-51cb0132.pth')
		net.load_state_dict(pretrained_weight)
...
class Training:
	def __init__(self, model, device, config, name, fold_num, imsize, weight_dir):
		self.config = config
		self.epoch = 0
		self.base_dir = weight_dir+'/'
		self.model = model
		self.best_loss = 10**5
		self.device = device
		self.name = name
...
if __name__ == '__main__':
	...
	parser.add_argument('--name', type=str, deafult='effi4_0223', help='model save name')
	# for train.sh
	parser.add_argument('--data', type=str, default='../../../DATA/TEETH/TRAIN', help='original train image path')
	parser.add_argument('--label', type=str, default='../../../DATA/TEETH/train_label.json', help='original train json data')
	parser.add_argument('--weights', type=str, default='./weights', help='train base weight')
	...
	# 이미지 전처리 확인
	if not os.path.isdir('./src/images/train'):
		print('Train image preprocess')
		make_nochange_onlyTrain(args.data, args.label)

	# train.csv 확인
	if not os.path.isfile('./src/train.csv'):
		print('Make train.csv')
		mk_train_csv(args.data, args.label)
	...
	for fold_num in range(5): # fold
		...
		# model load
		model = Train_Model(which_model=args.model, img_size=args.imsize, weight_dir=args.weights)
		model.to(device)
		# dataset (train / valid)
		...
		# training
		training = Training(model=model, device=device, config=setting_config, name=args.name, fold_num = fold_num, imsize=args.imsize, weight_dir=args.weights)
