...
if __name__ == '__main__':
    ...
    # for test.sh
    parser.add_argument('--data', type=str, default='../../../DATA/TEETH/TEST', help='original test image path')
    parser.add_argument('--result', type=str, default='./results/result.json', help='result json')
	args = parser.parse_args()

	# 이미지 전처리 확인
    if not os.path.isdir('./src/images/test'):
        print('Test image preprocess')
        make_nochange_onlyTrain(args.data)
	...
	# dataset
	test_img_path = './src/images/test'
	test_image_names = np.array(os.listdir(test_img_path))
	test_dataset = LoadDataset(image_names = test_image_names, imsize = args.imsize, which = 'test',  root='./src/images/')
	test_loader = DataLoader(test_dataset, batch_size=args.batch, num_workers=args.nw, collate_fn=collate_fn, shuffle=False)

	# load weight & model
	checkpoint_ls = ['./weights/{}_{}_best.pt'.format(args.ckp_nm, i) for i in range(5)]
	...
	# json
	save_path = './results/{}.json'.format(args.result)
