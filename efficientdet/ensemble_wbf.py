...
#6개의 model result를 모두 모으기 (5 effi + 1 yolo)
def model6_predictions(image_names, images, targets, model_ls, score_threshold, device, imsize, json_data):
	test_img_path = '../dataset_nochange/images/test'
	with torch.no_grad():
		images = torch.stack(images).float().to(device)
		predictions = [] # 총 6개의 모델별 (이미지들의)prediction이 predictions 안에 존재
		...
		for image_name in image_names: 
			im = np.array(Image.open('{}/{}'.format(test_img_path, image_name)))

			img_data = json_data[image_name]
			bbox_ls = [] ; score_ls = [] ; label_ls = []
			for data in img_data:
				if data['confidence_score'] > score_threshold:
					label_ls.append(convtoeffi_label(data['class']))
					score_ls.append(data['confidence_score'])
					bbox_ls.append(data['coord'])

			json_transformed = json_transforms(imsize=imsize)
			transformed = json_transformed(image=im, bboxes=bbox_ls, labels=label_ls)
			transformed_bboxes = transformed['bboxes']
			result.append({
				'boxes':np.array(transformed_bboxes),
				'scores': np.array(score_ls),
...
if __name__ == '__main__':
	...
	parser.add_argument('--iou_thr', default=0.35, type=float, help='WBF iou threshold')
	...
	test_dataset = LoadDataset(image_names = test_image_names, imsize = 640, which = 'test')
	test_loader = DataLoader(test_dataset, batch_size=args.batch, num_workers=args.nw, collate_fn=collate_fn, shuffle=False)


	# yolo json
	# python3 detect.py --source ../dataset_Aug/images/test/ --weights ./runs/train/Aug12/weights/best.pt --save-txt --save-conf --conf-thres 0.25 --iou-thres 0.99 --name allboxes_0224
	# python3 submission_json.py --infer yolov5/runs/detect/allboxes_0224/labels --name yolo5_0224_v4_allboxes
	yolo_json_path = '../submission/yolo5_0224_v4_allboxes.json' # conf-thres 0.25 --iou-thres 0.99
	with open(yolo_json_path, 'r') as f:
		json_data = json.load(f)

	# load weight & model (effi)
	# yolov5의 경우 json자체에서 값을 뽑도록
	checkpoint_ls = ['models/{}_{}_best.pt'.format(args.ckp_nm, i) for i in range(5)]
	models = [Load_Model(checkpoint_path=weight, which_model='effi4', img_size=640).to(device) for weight in checkpoint_ls]	
	
	# Predict
	results = {}
	for (image_names, images, targets) in tqdm(test_loader, total=len(test_loader)):
		predictions = model6_predictions(image_names=image_names, images=images, targets=targets, model_ls=models, 
			score_threshold=args.obj_thr, device=device, imsize=640, json_data=json_data)
		for i, (image_name, image) in enumerate(zip(image_names, images)):
			H, W = np.array(Image.open('{}/{}'.format(test_img_path, image_name))).shape[:2]
			boxes, scores, labels = run_wbf(predictions=predictions, image_index=i, image_size=640,
